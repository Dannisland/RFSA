...
def catimg(data):
    ...
    for i in range(data.shape[0]):
        ...
        tmp2 = cv2.normalize(tmp, None, 0, 255, cv2.NORM_MINMAX)
...
def cal_channel_max_min(data):
    ...
    for i in range(B):
        for j in range(C):
            channel = data[i][j]
            max_channel, min_channel = np.max(channel), np.min(channel)
            data_max.append(max_channel)
            data_min.append(min_channel)

    max_ = torch.tensor(np.array(np.split(np.array(data_max), B)))
    min_ = torch.tensor(np.array(np.split(np.array(data_min), B)))
...
if __name__ == "__main__":
    ...
    ################# 老师测试 #####################
    # path = '/workshop/xiaobin/ABDH/realimg.png'
    path = "../testimg/lenna.png"
    img = cv2.imread(path)
    ...
    img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img_ycbcr = rgb2ycbcr(img_rgb)  # rgb to ycbcr
    img_all = np.transpose(img_ycbcr, [2, 0, 1])
    img_dct = DCT_Block(img_ycbcr)
    img_rebuild = DCT_Rearrange(img_dct)
    # img_rebuild = DCT_Block()
    catimg(img_rebuild)
    #
    # img_y = img_all[0]
    # img_cb = img_all[1]
    # img_cr = img_all[2]
    #
    # cv2.imwrite('ycbcr.png', img_ycbcr)
    # cv2.imwrite('y.png', img_y)
    # cv2.imwrite('cb.png', img_cb)
    # cv2.imwrite('cr.png', img_cr)
    # cv2.imshow('1', img_ycbcr)
    # cv2.waitKey(0)
