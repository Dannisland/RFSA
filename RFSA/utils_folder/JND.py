...
def conv(input, filter, padding="SAME", rotate=False):
    ...
    for i in range(f_h // 2, h - f_h // 2 + 2 * pad_h):
        for j in range(f_w // 2, w - f_w // 2 + 2 * pad_w):
            # print(i, j)
            output[i - f_h // 2, j - f_w // 2] = np.sum(
                input_temp[i - f_h // 2: i + f_h // 2 + 1, j - f_w // 2: j + f_w // 2 + 1] * filter)
    return output
...
def func_edge_protect(img):
    ...
    if edge_threshold > 0.8:
        edge_threshold = 0.8
    edge_region = feature.canny(img, sigma=np.sqrt(2), low_threshold=0.4 * edge_threshold * 255,
                                high_threshold=edge_threshold * 255).astype(
        np.float32)  # NOTE: result little different from matlab
...
img = Image.open("../testimg/lena.png")
