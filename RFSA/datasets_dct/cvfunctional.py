...
def upscale(img, upscale_factor=None, desize_size=None, interpolation='BILINEAR'):
    ...
    if upscale_factor is not None:
        ...
        raise ValueError
    tmp = cv2.resize(img, dsize=(dw, dh), interpolation=INTER_MODE[interpolation])
    return tmp
...
def resized_crop(img, i, j, h, w, size, interpolation='BILINEAR'):
    ...
    # cv2.imwrite('/mnt/ssd/kai.x/work/code/iftc/datasets_dct/cvtransforms/test/crop.jpg', img)
    img = resize(img, size, interpolation)
    # cv2.imwrite('/mnt/ssd/kai.x/work/code/iftc/datasets_dct/cvtransforms/test/resize.jpg', img)
    return img
